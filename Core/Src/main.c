...
#include "../Application/Task_Default/task_default.h"
...
int main(void)
{
  ...
  osThreadDef(taskdefault, StartDefaultTask, osPriorityNormal, 0, 128);
  taskdefaultHandle = osThreadCreate(osThread(taskdefault), NULL);

  /* definition and creation of taskBuzzer */
  osThreadDef(taskBuzzer, startTaskBuzzer, osPriorityIdle, 0, 128);
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  HAL_GPIO_WritePin(GPIOB, RELAY_Pin|DP2_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOA, i2c_scl_Pin|i2c_sda_Pin|DIPS1_A_Pin|DIPS2_A_Pin
                          |DIPS3_A_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pins : RELAY_Pin DP2_Pin */
  GPIO_InitStruct.Pin = RELAY_Pin|DP2_Pin;
  ...
  /*Configure GPIO pins : i2c_scl_Pin i2c_sda_Pin DIPS1_A_Pin DIPS2_A_Pin
                           DIPS3_A_Pin */
  GPIO_InitStruct.Pin = i2c_scl_Pin|i2c_sda_Pin|DIPS1_A_Pin|DIPS2_A_Pin
                          |DIPS3_A_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
  /* USER CODE BEGIN Callback 0 */
	...
	if(zaman_durdur == 73){
		zaman_count100++;
			if(zaman_count100 == 100)
			{
			zaman_count++;
			zaman_count100 = 0;
			}
			if((zaman_count & 0x0a)== 0x0a)
			{
				zaman_count += 6;

				if((zaman_count & 0xa0) == 0xa0){
					zaman_count += 0x60;
					if((zaman_count & 0xa00) == 0xa00){
						zaman_count = 0;
					}
				}
			}
